...
from models.common import (SPP, Bottleneck, BottleneckCSP, Concat, Conv,
                           DWConv, Focus)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = x[i].sigmoid()
                y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 +
                               self.grid[i].to(x[i].device)) * self.stride[i]  # xy # noqa: E501
                y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * \
...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None):
        ...
        if isinstance(m, Detect):
            ...
            m.stride = torch.tensor(
                [s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))])  # forward # noqa: E501
    ...
    def forward(self, x, augment=False, profile=False):
        if augment:
            ...
            for si, fi in zip(s, f):
                ...
                # cv2.imwrite('img%g.jpg' % s, 255 * xi[0].numpy().transpose((1, 2, 0))[:, :, ::-1])  # save # noqa: E501
                yi[..., :4] /= si  # de-scale
                if fi == 2:
    ...
    #             print('%10.3g' % (m.w.detach().sigmoid() * 2))  # shortcut weights # noqa: E501

    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        print('Fusing layers... ', end='')
        for m in self.model.modules():
            if type(m) is Conv:
                m._non_persistent_buffers_set = set()  # pytorch 1.6.0 compatability # noqa: E501
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    anchors, nc, gd, gw = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple']  # noqa: E501
    na = (len(anchors[0]) // 2) if isinstance(anchors,
                                              list) else anchors  # number of anchors # noqa: E501
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        if m in [nn.Conv2d, Conv, Bottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, C3]:  # noqa: E501
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    # Profile
    # img = torch.rand(8 if torch.cuda.is_available() else 1, 3, 640, 640).to(device) # noqa: E501
    # y = model(img, profile=True)

    # ONNX export
    # model.model[-1].export = True
    # torch.onnx.export(model, img, opt.cfg.replace('.yaml', '.onnx'), verbose=True, opset_version=11) # noqa: E501

    # Tensorboard
    # from torch.utils.tensorboard import SummaryWriter
    # tb_writer = SummaryWriter()
    # print("Run 'tensorboard --logdir=models/runs' to view tensorboard at http://localhost:6006/") # noqa: E501
    # tb_writer.add_graph(model.model, img)  # add model to tensorboard
    # tb_writer.add_image('test', img[0], dataformats='CWH')  # add model to tensorboard # noqa: E501
